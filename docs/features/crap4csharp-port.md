...
| S12 — Differential-eval hardening | A one-time two-oracle differential experiment (gpt-5.6-sol × claude-opus-4.8, each blindly authoring ~25 test cases, cross-predicting the other's, then capturing ground truth), reconciled by JARVIS + Anders, surfaced and **empirically confirmed a real gate-evasion false-negative — coverlet #507**: coverlet is line-level and records only the FIRST accessor on a shared physical source line, dropping the rest from the cobertura XML entirely. So an **uncalled same-line SECOND accessor with CC ≥ 3** renders `N/A`, is excluded from `MaxCrap`, and **silently passes the exit-2 gate** (confirmed for property `{get;set}` [expr & block], indexer get/set, and event add/remove via `gate_probe*.ps1`; multi-line accessors/finalizers/operators/standalone getters are all caught at real 0%). No coverlet config toggle exists. Mr. Das ruled: **(T30)** surgical fix (option vi) — same-line-accessor 0%-promotion; **(T31)** two `decisions.md` doc corrections. A **zero-tests-executed fail-fast** was designed-analyzed (H3) and ruled **DON'T-ADD** (Mr. Das): no crap4java analog (net-new product rule), no detection signal in the faithful `ICommandExecutor.Run → int` pipeline (stdout never captured), and it false-couples to departure #10's unit-only filter — CI already owns "did tests run." Tasks: T30–T31. | S11 |
...
| T29 | S11 | **Property-based `ProjectReference` resolution** (S6 re-run #3, finding #7 — fast-follow). `TestProjectResolver.DirectProjectReferences` did a raw `Path.Combine(csprojDir, Include)`, so any `$(Prop)` token in a `ProjectReference Include` → bogus path → `File.Exists` false → edge dropped → transitive closure misses → false **"No test project" exit 1**. **Mr. Das ruled O2 (intrinsic + bounded user-defined), full ten intrinsic props.** Two-stage, no public API change: (1) fixpoint-expand a property map — base = the **ten** reserved path-derived props (`MSBuildThisFileDirectory` [trailing sep, load-bearing], `MSBuildProjectDirectory` [no sep], `…FullPath`/`…File`/`…FileName`/`…Extension` `MSBuildThisFile*`+`MSBuildProject*` families; names matched `OrdinalIgnoreCase`, paths stay `Ordinal` per #3) + user-defined props from the csproj `<PropertyGroup>`s and an **all-ancestors `Directory.Build.props` walk bounded to the invocation root** (nearest-wins; csproj body overrides); (2) O1's **single-pass** regex substitution (`[GeneratedRegex]`, `CoberturaCoverageParser` idiom) on each Include. `Condition=` honored ONLY for the narrow self-emptiness guard `Condition="'$(X)'==''"` (the ubiquitous `RepoRoot` idiom); all other conditioned props skipped. **`$(SolutionDir)` explicit-def only** — `.sln`-synthesis stays a documented fail-safe residual (honors departure #9's `.sln` retirement; finding #7 named `$(RepoRoot)`/`$(MSBuildThisFileDirectory)`, not `$(SolutionDir)`). **Fail-safe preserved:** any undefined/unevaluable/conditioned/cyclic/depth-capped token → retained VERBATIM → non-existent path → dropped → exit 1; never throws (per-`Directory.Build.props` `XmlException` swallowed like the existing probe, FS faults propagate to the T20 catch-all), never guesses a match. Cycle/self-ref + depth-cap safe; per-call memo (no static state). Folds under **departure #9** as `D-T29a` (no new number); rewords D-T28c → resolved for the intrinsic + bounded user-defined subset, residuals (`.sln`-`$(SolutionDir)`, non-self-guard conditions, property functions/item-metadata/env-global props, `<Import>` chains, props above root) deferred fail-safe. **Landed green — 33/33 resolver tests (14 existing + 19 new: 16 T29 + 3 Anders-nit polish — self-guard name-match, reversed/negated guard-form exclusion, nearer-props-over-farther precedence), full suite 197/197, 0/0 Release, CI green.** | Done | `cde2675` |
| T30 | S12 | **Same-line-accessor coverlet-blind 0%-promotion** (option vi — closes the coverlet #507 gate-evasion false-negative; **new departure #15**, C#-ecosystem, no crap4java analog). **Detection (syntax-only, in `CSharpMethodParser.EmitAccessors`):** an emitted (bodied) accessor is `CoverletBlind` iff its declaration start-line (`accessor.GetLocation()…StartLinePosition.Line + 1`, i.e. the accessor-keyword line, NOT the body end) equals the start-line of an EARLIER accessor of the same member — first-on-line wins, second+ is blind. Kind-general (property/indexer `get`/`set`/`init`, event `add`/`remove`); auto/bodyless accessors are already skipped by #14 (never descriptors → never blind); single-accessor members never blind. Detection may safely OVER-flag (superset-safe) because promotion is separately gated on genuine absence. **Plumbing:** append one trailing `bool CoverletBlind = false` to `MethodDescriptor` (2nd C#-only appended-last field after `TypeName`; `Emit` gains a trailing `bool coverletBlind = false` forwarded only by `EmitAccessors`, every other call site defaults false); **`MethodMetrics` UNCHANGED** — the bit is consumed inside `Analyze` before the metric is built, so by output time coverage is already a real 0.0. **Promotion (`CrapAnalyzer.Analyze`, between `LookupCoverage` and `CrapScore.Calculate`):** `coverage is null && method.CoverletBlind && EnclosingTypePresent(coverageMap, TypeName, sourceFile) → coverage = 0.0` (real 0% → CRAP → into `MaxCrap`/gate). **Type-present guard INCLUDED** (Mr. Das) = ∃ map key `StartsWith(TypeName + "#", Ordinal)` && `Contains("#" + basename + ":", Ordinal)` — asserts the enclosing type WAS instrumented; only suppresses the pathological type-entirely-absent case (keeps within W4's spirit). Fires strictly after exact+nearest both miss. **Blast radius:** name-mangling residuals (explicit-interface, non-literal/foreign `[IndexerName]` per #14(b)/T28, async/state-machine) are NOT coverlet-blind → stay `N/A`, untouched; multi-line accessors/finalizers/operators/standalone getters still caught at real 0%. **Accepted new false-FAIL (fail-closed, Mr. Das):** a COVERED same-line CC ≥ 3 second accessor whose hits coverlet mis-attributes to the first → absent → promoted 0% → false exit 2 (needs the same rare same-line + CC≥3 construction as the escape it closes). Tests: parser unit **P-b1..8** (blind flag: expr/block/reverse-order/body-wrap/indexer/event/get-only), analyzer unit **A-b1..5** (promotion incl. guard-miss and already-present no-op cases), minimal integration smoke via `gate_probe*.ps1` (every escape → exit 2; CC1/CC2 same-line second accessor → CRAP ≤ 6 → exit 0). Records **departure #15** + **D-T30a** (detection) / **D-T30b** (plumbing) / **D-T30c** (promotion + guard) / **D-T30d** (tests + accepted false-fail). Dave confirmed **#15** as next-free at record time. **Residual (non-blocking, Anders §2):** an *attributed* same-line accessor reads its StartLine at the attribute-list line (not the `get`/`set` keyword), so it could under-detect and revert to the pre-existing N/A escape — astronomically rare (attributed same-line pair, CC≥3) and status-quo, not a new defect. | Done | `3f8deda` |
| T31 | S12 | **Two `decisions.md` doc corrections** (differential-eval H2; doc-only, no code/test). **(a)** Departure #10's rationale is empirically WRONG — it claims an all-`type=IntegrationTests` suite "yields no coverage → the report fail-fast fires"; SOL-25 disproved this: coverlet instruments the target regardless of which tests match, so such a suite still produces a **populated all-0% cobertura report** (`coverageMap.Count > 0`), **no fail-fast fires**, and the run exits **0 unless a member's CRAP > 8**. Cross-reference trigger-2 map-count semantics (D-T14b) so the two read consistently. **(b)** Add a caveat that CRAP **magnitudes are NOT comparable to crap4java's**: coverlet counts line/sequence points (incl. braces/signature) whereas JaCoCo counts INSTRUCTION coverage — the CRAP algorithm is identical but absolute coverage % (and therefore CRAP) differ; do not compare crap4csharp CRAP numbers to crap4java's on the same logical code (aligns with the locked "Coverage" row / R1). | Done | `3f8deda` |
